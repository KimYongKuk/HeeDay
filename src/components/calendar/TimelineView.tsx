...
export function TimelineView({
  ...
  return (
    <div className="bg-surface min-h-0 flex-1 overflow-auto">
      <div className="relative flex min-w-[900px] flex-col">
        <div
          className="border-line bg-surface sticky top-0 z-10 grid h-10 shrink-0 border-b"
          style={{ gridTemplateColumns: `${LABEL_W}px minmax(0, 1fr)` }}
        >
          <div className="border-line text-ink-faint flex items-center border-r pl-4 text-[11.5px] font-medium">
            프로그램 {rows.length}
          </div>
          <div
            className="grid"
            style={{ gridTemplateColumns: `repeat(${win.weekStarts.length}, minmax(0, 1fr))` }}
          >
            {win.weekStarts.map((d) => (
              <div
                key={d}
                className={cn(
                  'border-hairline text-ink-faint flex items-center border-l pl-2 text-[11.5px] font-medium',
                  Number(d.slice(8, 10)) <= 7 && 'text-ink font-semibold',
                )}
              >
        ...
        {rows.length === 0 ? (
          <p className="text-ink-faint p-6 text-[13px]">이 기간에 해당하는 프로그램이 없습니다.</p>
        ) : (
          rows.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className="border-line grid h-[84px] border-b"
                style={{ gridTemplateColumns: `${LABEL_W}px minmax(0, 1fr)` }}
              >
                <div className="border-line flex flex-col gap-[3px] border-r px-4 py-3.5">
                  <Link
                    href={`/programs/${p.id}`}
                    className="hover:text-brand flex items-center gap-2 text-[13.5px] font-semibold"
                  >
                    <span
                  ...
                  <span className="text-ink-faint pl-4 text-[11.5px]">
                    {formatShort(p.startDate)} – {formatShort(p.endDate)} · 할 일 {p.doneCount}/
                    {p.taskCount}
                  </span>
                  {p.assignee ? (
                    <span className="text-ink-faint pl-4 text-[11.5px]">담당 {p.assignee}</span>
                  ) : null}
                </div>
                <div
                  className="relative"
                  style={{
                    backgroundImage: 'linear-gradient(to right, #efede8 1px, transparent 1px)',
                    backgroundSize: `${100 / win.weekStarts.length}% 100%`,
                  }}
                >
                  <div
                  ...
                  {list.map((t) => (
                    <TaskPopover key={t.id} task={t}>
                      <div
                        ...
                        className="text-ink-muted absolute top-14 flex -translate-x-1 items-center gap-1 text-[10.5px]"
        ...
        {todayInWindow ? (
          <>
            <div
              className="bg-brand pointer-events-none absolute top-0 bottom-0 w-[1.5px]"
              style={{ left: `calc(${LABEL_W}px + (100% - ${LABEL_W}px) * ${todayPct / 100})` }}
            />
            <div
              className="bg-brand pointer-events-none absolute top-11 -translate-x-1/2 rounded px-1.5 py-0.5 text-[10.5px] font-semibold text-white"
              style={{ left: `calc(${LABEL_W}px + (100% - ${LABEL_W}px) * ${todayPct / 100})` }}
            >
              오늘
            </div>
